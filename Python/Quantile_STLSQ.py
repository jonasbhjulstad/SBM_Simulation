...
class Quantile_STLSQ(BaseOptimizer):
    ...
    >>> from pysindy.optimizers import Quantile_STLSQ
    >>> lorenz = lambda z,t : [10*(z[1] - z[0]),
    >>>                        z[0]*(28 - z[2]) - z[1],
    >>>                        z[0]*z[1] - 8/3*z[2]]
    >>> t = np.arange(0,2,.002)
    >>> x = odeint(lorenz, [-8,8,27], t)
    >>> opt = Quantile_STLSQ(threshold=.1, alpha=.5)
    ...
    def __init__(
        ...
        super(Quantile_STLSQ, self).__init__(
            max_iter=max_iter,
        ...
        self.N_features = 0
        self.init = True
        self.quantile_regressor = QuantileRegressor(alpha=2.0, quantile=tau, solver='highs-ds')
        self.opt_iter = 0
    ...
    def init_casadi(self, N_samples, N_features):


        sym_y = cs.MX.sym('y', N_samples)
        sym_x = cs.MX.sym('x', N_samples)
        sym_coef = cs.MX.sym('coef')
        self.ipopt_opts = {"print_level": 0, "output_file": "", "file_print_level": 5, "max_iter": 100}
        self.nlpsol_opts = {"ipopt": self.ipopt_opts, "print_time": False}

        obj = 0

        coef_x = sym_x * sym_coef
        # conds = cs.if_else(sym_y > coef_x,
        #                 self.tau*cs.norm_2((sym_y - coef_x)),
        #                 (1-self.tau)*cs.norm_2((coef_x - sym_y)))

        eps_pos = cs.fmax(sym_y  - coef_x, 0)
        eps_neg = cs.fmax(coef_x - sym_y, 0)

        conds = self.tau * eps_pos + (1 - self.tau) * eps_neg

        for i in range(N_samples):
            obj += conds[i]


        self.F_obj = cs.Function('obj', [sym_x, sym_y, sym_coef], [obj])        
    ...
    def _regress(self, x, y):
        ...
        if (self.init):
            self.init = False
            self.init_casadi(x.shape[0], N_features)

        # self.ipopt_opts['output_file'] = "Logs/QSTLSQ_ipopt_" + str(self.opt_iter) + ".out"
        # self.opt_iter += 1
        # sym_coef = cs.MX.sym('coef')

        # coef = np.zeros(N_features)
        # for i in range(N_features):
        #     obj = self.F_obj(x[:,i], y, sym_coef)[0]

        #     prob = {'f': obj, 'x': sym_coef}

        #     solver = cs.nlpsol('solver', 'ipopt', prob, self.nlpsol_opts)
        #     # solver = cs.nlpsol('solver', 'bonmin', prob)

        #     sol = solver(x0=0.)
        #     coef[i] = sol['x'][0]

        # coef[i] = ridge_regression(x, y, self.alpha, **kw)
        # coef[i] = sol['x'].full().flatten()
        coef = np.zeros(N_features)
        for i in range(N_features):
            self.quantile_regressor.fit(x[:,i][:,np.newaxis], y)
            coef[i] = self.quantile_regressor.coef_
        self.iters += 1
        return coef
    ...
    def _reduce(self, x, y):
        ...
        for k in range(self.max_iter):
            ...
            warnings.warn(
                "Quantile_STLSQ._reduce did not converge after {} iterations.".format(
                    self.max_iter
                ),
                ConvergenceWarning,
            )
            try:
                ...
                coef = self.coef_
                warnings.warn(
                    "Quantile_STLSQ._reduce has no iterations left to determine coef",
                    ConvergenceWarning,
                )
        self.coef_ = coef
        self.ind_ = ind
