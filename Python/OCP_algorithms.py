...
def week_objective_solve(X_mean, U_mean, Wu, F_ODE, Nt, u_max = 0.1, u_min = 1e-6):
    Nx = X_mean.shape[1]
    Nu = 1

    X = cs.MX.sym('X', Nx, Nt)
    U = cs.MX.sym('U', Nu, int(Nt/7)+1)
    ...
    for i in range(Nt):
        if (i == 0):
            Xk = F_ODE(X_mean[0,:], U[week_idx])
        else:
            if (i % 7 == 0):
                week_idx += 1
            Xk = F_ODE(Xk, U[week_idx])
        X_traj.append(Xk)
        obj += Wu*(u_max - U[week_idx])**2 + (Xk[1])**2
    get_X = cs.Function('get_X', [U], [cs.horzcat(*X_traj)])

    X0 = X_mean
    U0 = U_mean[::7][:U.shape[0]]
    # prob = {'f': obj, 'x': W, 'g': g}
    # solver = cs.nlpsol('solver', 'ipopt', prob)
    lbx = [u_min]*U.shape[0]
    ubx = [u_max]*U.shape[0]
    ...
    return (sol, get_X(sol['x'].full()), np.array([np.repeat(sol['x'].full(), 7)]))
