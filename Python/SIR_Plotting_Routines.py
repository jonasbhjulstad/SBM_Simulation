...
def plot_sim_comparison_SIR(ax, X_list, X_mean, U_mean, x, u, t):
    ...
    for i in range(Nx):
        ...
        percentile_plot(ax[i], t, X_sim.T, label='Controlled MC-simulations')
...
def plot_sim_comparison_I(ax, X_list, X_mean, U_mean, x, u, t):
    ...
    percentile_plot(ax[0], t, X_sim.T, label='Controlled MC-simulations')
    ax[0].plot(t, x[1, :-1].T, color='k', linestyle='dotted', label='MPC')
    ax[0].plot(t, X_mean[:,1].T, color='k', linestyle='dashed', label='Uncontrolled mean')
    ax[-1].plot(t, U_mean, color='k', linestyle='dashed', label='Uncontrolled mean')
    ax[-1].plot(t, u.T, color='k', linestyle='dotted', label='MPC')
