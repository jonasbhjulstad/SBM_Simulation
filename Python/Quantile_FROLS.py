...
class Quantile_FROLS(BaseOptimizer):
    """Forward Regression Orthogonal Least-Squares (Quantile_FROLS) optimizer.
    ...
    max_iter : int, optional (default 10)
        ...
        the number of nonzero terms chosen by the Quantile_FROLS algorithm.
    ...
    history_ : list
        ...
        ``coef_`` at iteration k of Quantile_FROLS.
    ...
    >>> from pysindy.optimizers import Quantile_FROLS
    >>> lorenz = lambda z,t : [10 * (z[1] - z[0]),
    >>>                        z[0] * (28 - z[2]) - z[1],
    >>>                        z[0] * z[1] - 8 / 3 * z[2]]
    >>> t = np.arange(0, 2, .002)
    >>> x = odeint(lorenz, [-8, 8, 27], t)
    >>> opt = Quantile_FROLS(threshold=.1, alpha=.5)
    ...
    def __init__(
        ...
        super(Quantile_FROLS, self).__init__(
        ...
        self.alpha = alpha
        self.tau = tau
        self.quantile_regressor = QuantileRegressor(alpha=1.0, quantile=tau, solver='highs-ds')
    ...
    def quantile_error(self, xm, y, gm):
        err = 0
        for i in range(xm.shape[0]):
            if (gm*xm[i] < y[i]):
                err += self.tau*(y[i] - gm*xm[i])
            else:
                err += (1-self.tau)*(gm*xm[i] - y[i])
        return err

    def _select_function(self, x, y, sigma, skip=[]):
        n_features = x.shape[1]
        g = np.zeros(n_features)  # Coefficients to orthogonalized functions
        ERR = np.ones(n_features)*np.inf  # Error Reduction Ratio at this step
        for m in range(n_features):
            if (m not in skip) and (not np.allclose(x, 0)):
                g[m] = self.quantile_regressor.fit(x[:, m][:,np.newaxis], y).coef_
                ERR[m] = self.quantile_error(x[:,m], y, g[m]) # Error reduction
    ...
    def _reduce(self, x, y):
        ...
        greedy Forward Regression Orthogonal Least Squares (Quantile_FROLS) algorithm
        """
        ...
        if self.verbose:
            ...
            print(
                ...
                " as the loss function being minimized by Quantile_FROLS!"
            )
