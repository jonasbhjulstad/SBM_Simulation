...
def read_graph_data(p_dir):
    ...
    for g_dir in graph_dirs:
        ...
        vpms.append(np.genfromtxt(g_dir + "/vpm.csv", delimiter=","))
    return edgelists, ecms, vpms


def simulate_with_solution(q, p_dir, u_opt, N_sims):
    SBM_Database::Sim_Param = json.load(open(p_dir + "Sim_Param.json"))
    edge_lists, ecms, vpms = read_graph_data(p_dir)
    ...
    sim_param.N_graphs = len(vpms)
    # write json to file
    p_I_run(q, sim_param, edge_lists, ecms, vpms, N_connections, u_opt)
...
def optimize(Sim_dir, output_dir, N_sims, theta, Wu, u_min, u_max, Nt_per_u=7):
    ...
    ccmap = np.genfromtxt(connection_community_map_path,
                          delimiter=",", dtype=int)
    if (len(ccmap.shape) == 1):
        ccmap = np.array([ccmap])
    alpha, N_communities, N_connections, init_state, N_pop, Nt = load_data(
        Sim_dir, N_sims)

    Nu = int(ceil(Nt / Nt_per_u))
    u = MX.sym("u", Nu, N_connections)

    w_opt, traj, f = solve_single_shoot(
        ccmap, theta, alpha, N_communities, init_state, Nt, Wu, u, u_min, u_max, output_dir + "/IPOPT.log")
    u_opt = np.reshape(w_opt, (Nu, N_connections))

    u_uniform = MX.sym("u_single", Nu, 1)
    u_opt_uniform, traj_uniform, f_uniform = solve_single_shoot(
        ccmap, theta, alpha, N_communities, init_state, Nt, Wu, u_uniform, u_min, u_max, output_dir + "/Uniform_IPOPT.log")

    u_opt = np.repeat(u_opt, Nt_per_u, axis=0)[:Nt]
    # duplicate u_opt_uniform
    u_opt_uniform = np.repeat(np.repeat(u_opt_uniform, Nt_per_u, axis=0)[
                              :Nt, :], N_connections, axis=1)
    ...
    u_opt_comparison_plot(traj, u_opt, f, traj_uniform,
                          u_opt_uniform, f_uniform, output_dir)
...
def optimize_sim_dir(sim_dir, N_sims, tau):
    theta_LS, theta_QR, MSE, MAE = regression_on_datasets(
        sim_dir, params["N_sims"], tau, 0)
    assert not np.any(np.isnan(theta_LS)
                      ) and "nan in theta_LS for dir " + sim_dir
    assert not np.any(np.isnan(theta_QR)
                      ) and "nan in theta_QR for dir " + sim_dir
    ...
    optimize(sim_dir, sim_dir + "/LS/",
             params["N_sims"], theta_LS, Wu, params["p_I_min"], params["p_I_max"])
    optimize(sim_dir, sim_dir + "/QR/",
             params["N_sims"], theta_QR, Wu, params["p_I_min"], params["p_I_max"])
...
if __name__ == '__main__':
    ...
    tau = .9
    p_dirs = get_p_dirs(
        "/home/man/Documents/ER_Bernoulli_Robust_MPC/build/data/SIR_sim/Excitation")
    for regression_type in ["LS", "QR"]:
        ...
        for gd in graph_dirs:
            ...
            optimize_sim_dir(gd + "/Detected_Communities/",
                             params["N_sims"], tau)
