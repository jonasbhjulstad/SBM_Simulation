...
#include <Sycl_Graph/Dataframe/Buffer_Utils.hpp>



void validate_buffer_init_sizes(Sim_Param p, const Dataframe_t<std::pair<uint32_t, uint32_t>,2> &edge_list, const Dataframe_t<uint32_t,2> &vcms, const Dataframe_t<uint32_t, 2> &ecms, std::vector<float> p_Is_init)
{
...
Dataframe_t<float, 3> generate_duplicated_p_Is(uint32_t Nt, uint32_t N_sims_tot, const uint32_t N_connections_max, float p_I_min, float p_I_max, uint32_t seed)
{
    ...
    while (t < Nt)
    {
        Dataframe_t<float, 2> p_Is_t(N_sims_tot, N_connections_max);
        for (auto &p_I : p_Is_t)
        {
...
Dataframe_t<float, 3> Sim_Buffers::generate_random_p_Is(sycl::queue &q, Sim_Param p, pqxx::connection &con, const Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list_undirected, const Dataframe_t<uint32_t, 2> &vcms)
{
...
Sim_Buffers::Sim_Buffers(sycl::queue &q, Sim_Param p, pqxx::connection &con, const Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list_undirected, const Dataframe_t<uint32_t, 2> &vcms, Dataframe_t<float, 3> p_Is_init)
{
    ...
    auto ecms = Dataframe_t<uint32_t, 2>(ecms_from_vcms(edge_list, vcms, ccm_indices));
    ...
    if (!p_Is_init.size())
    {
        p_Is_init = generate_random_p_Is(q, p, con, edge_list_undirected, vcms);
    }
    ...
    write_ccm(con, p.p_out_idx, ccms);
