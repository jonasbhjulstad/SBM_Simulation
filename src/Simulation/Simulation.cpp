...
Simulation_t::Simulation_t(sycl::queue &q, pqxx::connection &con, const Sim_Param &sim_param, const Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list, const Dataframe_t<uint32_t, 2> &vcm, sycl::range<1> compute_range, sycl::range<1> wg_range, const Dataframe_t<float, 3> &p_Is)
    : q(q), con(con), p(sim_param), b(q, sim_param, con, edge_list, vcm, p_Is), compute_range(compute_range), wg_range(wg_range)
{
}

Simulation_t::Simulation_t(sycl::queue &q, pqxx::connection &con, const Sim_Param &sim_param, const Sim_Buffers &sim_buffers, sycl::range<1> compute_range, sycl::range<1> wg_range)
    : q(q), con(con), p(sim_param), b(sim_buffers), compute_range(compute_range), wg_range(wg_range)
{
}
...
void Simulation_t::write_allocated_steps(uint32_t t, std::vector<sycl::event> &dep_events, uint32_t N_max_steps)
{
    ...
    write_graphseries(con, p.p_out_idx, state_df, "community_state", t - p.Nt_alloc);
    write_graphseries(con, p.p_out_idx, event_df, "connection_events", t - p.Nt_alloc);
    state_df.resize_dim(2, N_steps + 1);
    event_df.resize_dim(2, N_steps);
    auto inf_gs = sample_infections(state_df, event_df, b.ccm, p.seed);
    write_graphseries(con, p.p_out_idx, inf_gs, "infection_events", t - p.Nt_alloc);
    t2 = std::chrono::high_resolution_clock::now();
    std::cout << "Inf sample/ write graphseries: " << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() << "ms\n";
}

void Simulation_t::write_initial_steps(sycl::queue &q, const Sim_Param &p, pqxx::connection &con, Sim_Buffers &b, std::vector<sycl::event> &dep_events)
{
