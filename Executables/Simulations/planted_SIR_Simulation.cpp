...
void simulate_to_file(Network_t &network, const tp_list_t &tp_list,
  ...
  auto [tot_traj, delta_Is, delta_Rs] = network.simulate_groups(tp_list);

  auto filewrite([&](std::ofstream &file, const auto &traj) {
    std::for_each(traj.begin(), traj.end(), [&](auto &t_i) {
      std::for_each(t_i.begin(), t_i.end(),
...
std::vector<Partition_Edge_List_t>
random_connect(const std::vector<Nodelist_t> &nodelists,
...
auto create_SIR_Bernoulli_SBM(sycl::queue &q, const std::vector<uint32_t> N_pop,
  ...
  std::vector<Partition_Edge_List_t> edge_id_lists =
      random_connect(nodelists, p_SBM, true, undirected, N_threads, seed);
  uint32_t N_edges =
      std::accumulate(edge_id_lists.begin(), edge_id_lists.end(), 0,
                      [](const auto &sum, auto &edge_id_list) {
                        return sum + edge_id_list.size();
                      });

  std::vector<std::pair<uint32_t, uint32_t>> edge_ids(N_edges);
  // flatten edge_id_lists
  std::for_each(edge_id_lists.begin(), edge_id_lists.end(),
                [&](auto &edge_id_list) {
                  edge_ids.insert(edge_ids.end(), edge_id_list.begin(),
                                  edge_id_list.end());
                });
  ...
  // create network
  Network_t sir(G, p_I0, p_R0, edge_id_lists, nodelists, seed);

  return sir;
}

auto create_SIR_Bernoulli_planted(sycl::queue &q, uint32_t N_pop, uint32_t N,
                                  float p_in, float p_out, float p_I0,
                                  float p_R0 = 0.0, bool undirected = true,
                                  uint32_t N_threads = 4, uint32_t seed = 47) {
  ...
  return create_SIR_Bernoulli_SBM(q, N_pop_vec, p_SBM, p_I0, p_R0, undirected,
                                  N_threads, seed);
}

auto create_SIR_Bernoulli_planted(uint32_t Ng, sycl::queue &q, uint32_t N_pop,
                                  uint32_t N, float p_in, float p_out,
                                  float p_I0, float p_R0 = 0.0,
                                  bool undirected = true, uint32_t seed = 47) {
  std::vector<std::pair<std::vector<Network_t>, uint32_t>> net_seed(Ng);
  std::mt19937 rd(seed);
  std::generate(net_seed.begin(), net_seed.end(), [&rd]() {
    return std::make_pair(std::vector<Network_t>(), rd());
  });
  std::for_each(std::execution::par_unseq, net_seed.begin(), net_seed.end(),
                [&q, N_pop, N, p_in, p_out, p_I0, p_R0, undirected](auto &ns) {
                  auto sir = create_SIR_Bernoulli_planted(
                      q, N_pop, N, p_in, p_out, p_I0, p_R0, undirected, 4,
                      ns.second);
                  sir.initialize();
  ...
  for (auto &&network_vec : net_seed) {
    networks_flat.insert(networks_flat.end(), network_vec.first.begin(),
                         network_vec.first.end());
  }
...
int main() {
  ...
  auto networks = create_SIR_Bernoulli_planted(Ng, q, N_pop, N_clusters, p_in,
                                               p_out, p_I0, p_R0, seed);

  size_t byte_size = std::accumulate(
      networks.begin(), networks.end(), 0,
      [](const auto &sum, auto &network) { return sum + network.byte_size(); });

  std::cout << "Total network byte size: " << byte_size << std::endl;
  ...
  simulate_to_file(networks, tp_list,
                   std::string(Sycl_Graph::SYCL_GRAPH_DATA_DIR) + "/SIR_sim/");
  std::cout << "reads: " << networks[0].counter << std::endl;
