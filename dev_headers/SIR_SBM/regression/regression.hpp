...
connection_expand_population(const std::tuple<casadi::DM, casadi::DM> &data,
  ...
  size_t con_idx = 0;
  auto population_slice = [](size_t idx) {
    return Slice(idx * 3, idx * 3 + 3);
  };
  for (auto comb : iter::combinations_with_replacement(N_communities, 2)) {
    auto [from_idx, to_idx] = comb;
...
regression_data_from_simulations(const std::filesystem::path &filenameprefix,
  ...
  using namespace casadi;
  auto linvec_to_dm = [](const LinearVector3D &vec, size_t start, size_t end) {
  ...
  auto [population_sources, population_targets] = connection_expand_population(
      std::make_tuple(population_counts, infection_counts), N_communities,
      N_connections);
