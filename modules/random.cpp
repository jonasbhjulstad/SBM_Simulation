...
export module SIR_SBM.random;


export template <typename RNG>
std::vector<RNG> generate_rngs(uint32_t seed, uint32_t N) {
  RNG seeder(seed);
...
export std::vector<uint32_t> discrete_finite_sample(std::mt19937_64& rng, const std::vector<uint32_t>& weights, uint32_t N_samples) {
