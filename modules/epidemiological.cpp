...
import SIR_SBM.sycl.buffer;
import SIR_SBM.exceptions;
#include <oneapi/dpl/random>

export module SIR_SBM.epidemiological;

export enum class SIR_State : char {
  Susceptible = 0,
  Infected = 1,
  Recovered = 2,
  Invalid = 3
...
export struct Population_Count {
  int S, I, R;
...
export Population_Count state_to_count(SIR_State s) {
  switch (s) {
  case SIR_State::Susceptible:
...
export sycl::event initialize(sycl::queue &q, sycl::buffer<SIR_State, 3> &state,
...
export sycl::event state_copy(sycl::queue &q, sycl::buffer<SIR_State, 3> &state,
...
export sycl::event recover(sycl::queue &q, sycl::buffer<SIR_State, 3> &state,
...
export sycl::event infect(sycl::queue &q, sycl::buffer<SIR_State, 3> &state,
