...
import SIR_SBM.population_count;
import SIR_SBM.simulation.result;
import SIR_SBM.csv;
import SIR_SBM.vector;
import SIR_SBM.occurrence;
...
sample_infections(const std::shared_ptr<uint32_t> &contact_events,
  ...
  std::vector<uint32_t> connection_contacts = get_partition_connection_contacts(
      get_column(contact_events, {sim_idx, t_idx},
                 {N_sims, N_connections * 2, Nt}),
      N_partitions, p_idx);
...
export std::shared_ptr<uint32_t>
sample_infections(const std::shared_ptr<uint32_t> &contact_events,
                  const std::shared_ptr<Population_Count> &population_count,
  ...
  for (int sim_idx = 0; sim_idx < N_sims; sim_idx++) {
    for (int t_idx = 0; t_idx < Nt; t_idx++) {
      for (int p_idx = 0; p_idx < N_partitions; p_idx++) {
        infections_pt = sample_infections(
            contact_events, population_count, {sim_idx, p_idx, t_idx},
            {N_sims, N_connections, N_partitions, Nt}, rngs[sim_idx]);
