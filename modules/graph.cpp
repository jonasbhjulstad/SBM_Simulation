...
import SIR_SBM.combination;
import SIR_SBM.random;
...
export uint32_t bipartite_max_edges(uint32_t N0, uint32_t N1) { return N0 * N1; }
...
export std::vector<Vertexlist_t> SBM_vertices(uint32_t N_pop, uint32_t N_communities) {
...
export SBM_Graph generate_planted_SBM(uint32_t N_pop, uint32_t N_communities,
  ...
  auto rngs =
      generate_rngs<oneapi::dpl::ranlux48>(seed, n_choose_k(N_communities, 2));
