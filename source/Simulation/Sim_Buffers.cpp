...
Sim_Buffers::Sim_Buffers(sycl::queue &q, const SBM_Database::Sim_Param &p,
    ...
    : rngs{Buffer_Routines::generate_rngs(q, p.N_sims, p.seed)},
      vertex_state{sycl::buffer<SIR_State, 3>(
          sycl::range<3>(p.N_sims, p.Nt_alloc + 1, p.N_pop * p.N_communities))},
      accumulated_events{sycl::buffer<uint32_t, 3>(
          sycl::range<3>(p.N_sims, p.Nt_alloc, p.N_connections))},
      ...
      community_state{sycl::buffer<State_t, 3>(
          sycl::range<3>(p.N_sims, p.Nt_alloc + 1, p.N_communities))},
