...
sycl::event recover(sycl::queue &q, const SBM_Database::Sim_Param &p,
                    ...
                    sycl::range<1> compute_range, sycl::range<1> wg_range,
...
sycl::event infect(sycl::queue &q, const SBM_Database::Sim_Param &p,
                   Sim_Buffers &b, uint32_t t, sycl::range<1> compute_range,
                   sycl::range<1> wg_range, sycl::event &dep_event) {
  ...
  Buffer_Routines::validate_buffer_elements<float, 3>(
      q, b.p_Is,
      [p](auto elem) { return elem >= p.p_I_min && elem <= p.p_I_max; });
                     ...
                     for (uint32_t edge_idx = 0;
                       ...
                       if ((v_prev_from == SIR_INDIVIDUAL_S) &&
                         ...
                         if (bernoulli_I(rng)) {
                           ...
                           event_acc[gid][0][connection_id] += 1;
                         }
                       }
                     }
                   });
