...
typedef hipsycl::sycl::accessor<int, 2, hipsycl::sycl::access::mode::read_write, hipsycl::sycl::access::target::local>
    LocalBlock;
...
void BlockedFloydWarshallCompute(hipsycl::sycl::nd_item<1> &item, const LocalBlock &C,
  ...
  for (int k = 0; k < block_length; k++) {
    ...
    item.barrier(hipsycl::sycl::access::fence_space::local_space);
  }
}
...
void BlockedFloydWarshallPhase1(hipsycl::sycl::queue &q, int *graph, int round) {
  ...
  q.submit([&](hipsycl::sycl::handler &h) {
    LocalBlock block(hipsycl::sycl::range<2>(block_length, block_length), h);

    h.parallel_for<class KernelPhase1>(
        hipsycl::sycl::nd_range<1>(blocks * block_size, block_size), [=](hipsycl::sycl::nd_item<1> item) {
          ...
          graph[(std::round * block_length + i) * nodes +
                (std::round * block_length + j)] = block[i][j];
          item.barrier(hipsycl::sycl::access::fence_space::local_space);
        });
  });
...
void BlockedFloydWarshallPhase2(hipsycl::sycl::queue &q, int *graph, int round) {
  // Each group will process one block.
    ...
    h.parallel_for<class KernelPhase2>(
        nd_range<1>(blocks * block_size, block_size), [=](nd_item<1> item) {
          ...
          if (index != round) {
            ...
            // Copy data to local memory.
            diagonal[i][j] = graph[(std::round * block_length + i) * nodes +
                                   (std::round * block_length + j)];
            ...
            graph[(index * block_length + i) * nodes +
                  (std::round * block_length + j)] = off_diag[i][j];

            // Copy data to local memory.
            off_diag[i][j] = graph[(std::round * block_length + i) * nodes +
...
void BlockedFloydWarshall(hipsycl::sycl::queue &q, int *graph) {
