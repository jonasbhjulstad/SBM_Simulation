    ...
    namespace Sycl::Network_Models
    {
            ...
            : public Network<SIR_Bernoulli_Param<>, SIR_Bernoulli_Network>
        {
            ...
            SIR_Bernoulli_Network(Graph_t &G, float p_I0, float p_R0, uint32_t N_neighbors_max = 1, int seed = 777)
                : Base_t(3), q(G.C.q), G(G), p_I0(p_I0), p_R0(p_R0), N_neighbors_max(N_neighbors_max), seed_buf(sycl::range<1>(G.NV))
            {
                assert(G.NV > 0);
            ...
            void initialize()
            {
                ...
                q.submit([&](sycl::handler &h)
                    ...
                    auto v = G.C.vertex_buf.template get_access<sycl::access::mode::write>(h);
                    ...
                    if(d_I(rng) < p_I0)
                        {
                            v[i] = SIR_INDIVIDUAL_I;
                        }
                        else if(d_R(rng) < p_R0)
                        {
                            v[i] = SIR_INDIVIDUAL_R;
                        }
                        else
                        {
                            v[i] = SIR_INDIVIDUAL_S;
                        }
                }); });
            }
            ...
            void infection_step(float p_I)
            {
                ...
                // // sort edges by edge_cond
                // q.submit([&, this](sycl::handler &h)
                //          {
                //     auto e_acc = G.edge_access(h);
                //     auto e_to_acc = G.edge_to_access(h);
                //     auto e_from_acc = G.edge_from_access(h);
                //     auto v_acc = G.vertex_access(h);
                //     // parallel for
                //     h.parallel_for<class bitonic_sort>(cl::sycl::range<1>(e_acc.size()), [=](cl::sycl::id<1> index)
                //                                        {
                //         // get the index of the element to sort
                //         int i = index[0];

                //         // bitonic sort
                //         for (int k = 2; k <= e_acc.size(); k *= 2)
                //         {
                ...
                //                 if (ixj > i)
                //                 {
                //                     uint32_t idx_hi = ((i&k) == 0) ? i : ixj;
                //                     uint32_t idx_lo = ((i&k) == 0) ? ixj : i;
                //                     if (sus_inf_edge_comp(v_acc, e_to_acc[idx_lo], e_from_acc[idx_lo], e_to_acc[idx_hi], e_from_acc[idx_hi]))
                //                     {
                //                         std::swap(e_acc[idx_hi], e_acc[idx_lo]);
                //                         std::swap(e_to_acc[idx_hi], e_to_acc[idx_lo]);
                //                         std::swap(e_from_acc[idx_hi], e_from_acc[idx_lo]);
                //                     }
                //                 }
                //             }
                //          }}); });
                ...
                q.submit([&, this](sycl::handler &h)
                    ...
                    // parallel for
                    h.parallel_for<class edge_validity>(cl::sycl::range<1>(e_to_acc.size()), [=](cl::sycl::id<1> index)
                                                       {
                        // get the index of the element to sort
                        int i = index[0];
                        uint32_t sn = get_susceptible_neighbor(v_acc, e_to_acc[i], e_from_acc[i]);
                ...
                q.submit([&](sycl::handler &h)
                    ...
                    h.parallel_for(sycl::range<1>(sn_ids_acc.size()), [=](sycl::id<1> id)
                    {
                        ...
                        if (v_acc[sn_ids_acc[id[0]]] == SIR_INDIVIDUAL_S)
                        {
                            if (d_R(rng) < p_I)
                            {
                                v_acc[sn_ids_acc[id[0]]] = SIR_INDIVIDUAL_I;
                            }
                        }
                        seed_acc[id] += 1;
                    }); });
                std::vector<SIR_Individual_State> v(G.N_vertices + 1);
                sycl::buffer<SIR_Individual_State> v_host_buf(v.data(), sycl::range<1>(v.size()));
                q.submit([&](sycl::handler &h)
                         {
                    auto v_acc = G.vertex_access(h);
                    auto v_host_acc = v_host_buf.template get_access<sycl::access::mode::write>(h);
                    h.copy(v_acc, v_host_acc); });
                q.wait();
