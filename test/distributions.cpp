...
int main()
{
    ...
    Sycl_Graph::random::uniform_real_distribution<float> dist(0, 1);
    for (size_t i = 0; i < N_rngs; ++i)
    {
        std::cout << dist(mt) << std::endl;
    }

    Sycl_Graph::random::binomial_distribution<> dist2(10, 0.5);
    for (size_t i = 0; i < N_rngs; ++i)
    {
    ...
    sycl::buffer<Sycl_Graph::random::binomial_distribution<> , 1> dist_buf(N_rngs);
