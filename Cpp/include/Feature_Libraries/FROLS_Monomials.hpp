...
Vec generate_monomial(const Mat& X, const iVec& powers)
{
  Vec result(X.rows());
  for (int i = 0;i < powers.rows(); i++)
  {
    result *= X.row(i).pow(powers(i));
  }
  return result;
}

template <size_t Nx, size_t N_control>
Mat generate_monomials(const Mat& X, const iMat& powers)
{
  Mat result(X.rows(), X.cols());
  for (size_t i = 0; i < X.cols(); i++)
  {
    result.col(i) = generate_monomial(X, powers.row(i));
  }
  return result;
}
