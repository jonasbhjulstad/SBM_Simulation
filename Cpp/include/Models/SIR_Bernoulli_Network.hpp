...
void generate_SIR_ER_model(igraph_t &G, size_t N_pop,
                           double p_ER,
                           RNG &rng) {
...
template <typename RNG>
std::vector<SIR_State> generate_initial_infections(size_t N_pop, double p_I0, double p_R0, RNG& rng) {
    std::vector<SIR_State> state(N_pop);
    std::bernoulli_distribution d_I(p_I0);
    std::bernoulli_distribution d_R(p_R0);
    for (size_t i = 0; i < N_pop; i++) {
        state[i] = d_I(rng) ? I : S;
        state[i] = d_R(rng) ? R : state[i];
    }
    return state;
}
