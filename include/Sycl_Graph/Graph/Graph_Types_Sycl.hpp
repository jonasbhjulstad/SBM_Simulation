...
template <typename E, typename uI_t> struct Edge_Buffer {
  ...
  std::vector<Edge<E, uI_t>> get_edges()
  {
      ...
      q.submit([&](sycl::handler &h) {
          ...
          auto from_acc =
              from_buf.template get_access<sycl::access::mode::read>(h);
          auto data_acc =
              data_buf.template get_access<sycl::access::mode::read>(h);
          h.parallel_for<class edge_get_edges>(
              sycl::range<1>(N_edges), [=](sycl::id<1> id) {
                  if (to_acc[id] != invalid_id && from_acc[id] != invalid_id) {
                      edges.push_back({to_acc[id], from_acc[id], data_acc[id]});
                  }
              });
      });
      return edges;
  ...
  Edge_Buffer<E, uI_t> &operator+(const Edge_Buffer<E, uI_t> &other) {
    to_buf = device_buffer_combine(to_buf, other.to_buf, q, this->N_edges,
                                   other.N_edges);
    from_buf = device_buffer_combine(from_buf, other.to_buf, q, this->N_edges,
                                     other.N_edges);
    data_buf = device_buffer_combine(data_buf, other.to_buf, q, this->N_edges,
                                     other.N_edges);
    return *this;
  }
...
template <typename V, typename uI_t> struct Vertex_Buffer {
  ...
  Vertex_Buffer<V, uI_t> &operator+(const Vertex_Buffer<V, uI_t> &other) {
    uI_t N_free_vertices = this->size() - N_vertices;
    id_buf = device_buffer_combine(id_buf, other.to_buf, q, this->N_vertices,
                                   other.N_vertices);
    data_buf = device_buffer_combine(data_buf, other.to_buf, q,
                                     this->N_vertices, other.N_edges);
    return *this;
  }
