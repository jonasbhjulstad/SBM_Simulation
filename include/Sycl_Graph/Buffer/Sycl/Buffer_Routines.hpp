...
namespace Sycl_Graph
{
  ...
  template <typename T, std::unsigned_integral uI_t = uint32_t>
  void device_buffer_remove(sycl::buffer<T, 1> &buf, sycl::queue &q,
  ...
  template <typename T, std::unsigned_integral uI_t = uint32_t>
  void device_buffer_remove(sycl::buffer<T, 1> &buf, sycl::queue &q, const std::vector<uI_t> &indices)
  {
    if (indices.size() == 0)
    {
      return;
    ...
    if constexpr (sizeof(T) > 0)
    {
      ...
      auto acc = buf.template get_access<sycl::access::mode::write>(h);
      auto indices_acc =
          indices_buf.template get_access<sycl::access::mode::read>(h);
      h.single_task([=]() {
        uI_t offset = 0;
        for (uI_t i = 0; i < acc.size(); i++)
        {
          if (i == indices_acc[offset])
          {
            offset++;
          }
          else
