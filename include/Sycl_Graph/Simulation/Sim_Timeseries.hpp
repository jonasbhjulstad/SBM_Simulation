...
Dataframe_t<T, 4> read_3D_buffer(sycl::queue& q, sycl::buffer<T, 3>& buf, uint32_t N_graphs, std::vector<sycl::event> dep_events)
{
    ...
    Dataframe_t<T,4> dataframe(N_graphs, N_sims, Nt, N_columns);
    ...
    for(int sim_tot_idx = 0; sim_tot_idx < N_sims_tot; sim_tot_idx++)
    {
        ...
        for(int t = 0; t < Nt; t++)
        {
            ...
            {
                dataframe[graph_idx][sim_idx][t][c] = data[t*N_sims_tot*N_columns + graph_idx*p.N_sims*N_columns + sim_idx*N_columns + c];
            }
        }
    }
    return dataframe;
...
template <typename T>
void write_dataframe(const std::string& fname, const Dataframe_t<T, 2>& df, bool append = false)
{
    std::fstream f;
    if(append)
    {
    ...
    {
        for(int col = 0; col < df[row].size(); col++)
        {
            f << df({row, col}) << ",";
        }
        f << "\n";
...
template <typename T>
void write_dataframe(const std::string& basename, const Dataframe_t<T, 3>& df, bool append = false)
{
    for(int i = 0; i < df.size(); i++)
    {
        write_dataframe(basename + std::to_string(i) + ".csv", df[i], append);
    }
}

template <typename T>
void write_dataframe(const std::string& base_dir, const std::string& basename, const Dataframe_t<T, 4>& df, bool append = false)
{
    for(int i = 0; i < df.size(); i++)
    {
