...
private:
  ...
  infection_scatter(float dt) {
    ...
    if (N_edges > 0) {
      ...
      // print contents of e_inf_buf
      auto e_inf_acc =
          e_inf_buf.template get_access<sycl::access::mode::read>();
      // for (int i = 0; i < N_vertices; i++) {
        // std::cout << "e_inf_acc[" << i << "] = " << e_inf_acc[i] << std::endl;
      // }
    }
