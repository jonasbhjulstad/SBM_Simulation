...
namespace Sycl_Graph::Sycl
{
    ...
    struct Network
    {
        ...
        auto read_node_states(const TemporalParam tp, const std::vector<uint32_t>& ids)
        {
            return static_cast<Derived *>(this)->read_node_states(tp, ids);
        }
        ...
        std::vector<State> simulate(std::vector<TemporalParam> tp)
        {
            size_t Nt = tp.size()-1;
            std::vector<State> trajectory(Nt + 1);
            if (tp.size() < Nt+1)
        ...
        std::vector<std::vector<State>> simulate_nodes(std::vector<TemporalParam> tp, const std::vector<std::vector<uint32_t>>& ids)
        {
            size_t Nt = tp.size()-1;
            std::vector<std::vector<State>> trajectory(Nt + 1);
            uint32_t t = 0;

            TemporalParam tp_i = tp[0];
            trajectory[0] = read_node_states(tp_i, ids);
            for (int i = 0; i < Nt; i++)
            {
                advance(tp_i);
                trajectory[i + 1] = read_node_states(tp[i+1], ids);
                if (terminate(trajectory[i + 1], tp_i))
                {
                    break;
                }
