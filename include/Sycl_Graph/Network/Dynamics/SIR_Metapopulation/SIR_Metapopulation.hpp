...
#include <Sycl_Graph/Network/Dynamics/Network.hpp>
...
namespace Sycl_Graph::Network::Dynamics::SIR_Metapopulation
{
  ...
  struct Node
  {
    ...
    State(uint32_t S, uint32_t I, uint32_t R): S(S), I(I), R(R) {}
    ...
    {
      return State(S + other.S, I + other.I, R + other.R);
    }
    uint32_t S = 0.f;
    uint32_t I = 0.f;
    uint32_t R = 0.f;
  ...
  typedef Sycl_Graph::Graph::Invariant::Edge<void> Edge;

  struct Temporal_Param
  {
    float beta = 0;
    float alpha = 0;
  };

  struct Link: public Sycl_Graph::Network::Dynamics::Link<Edge, Temporal_Param, Link>
  {
    typedef Sycl_Graph::Network::Dynamics::Link<Edge, Temporal_Param, Link> Base_t;
    typedef Temporal_Param Temporal_Param_t;
    void advance(const Temporal_Param &tp, Base_t::From_Acc_t& from_acc, Base_t::To_Acc_t& to_acc)
    {
      auto &from = from_acc[0];
      auto &to = to_acc[0];
      auto S = from.S;
      auto I = from.I;
      auto R = from.R;
