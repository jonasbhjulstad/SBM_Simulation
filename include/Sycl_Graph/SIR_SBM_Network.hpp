...
struct SIR_SBM_Network {
  ...
  sycl::event infect(sycl::buffer<SIR_State> &state,
  ...
  accumulate_community_state(std::vector<sycl::buffer<State_t>> &result,
    ...
    assert(std::all_of(v_bufs.begin(), v_bufs.end(),
                       [&](auto &buf) { return buf.size() == N_vertices; }));
...
template <> struct sycl::is_device_copyable<Sycl_Graph::SBM::SIR_SBM_Network::State_t> : std::true_type {};
