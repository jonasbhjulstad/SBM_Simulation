...
namespace Sycl_Graph::SBM
{
  ...
  struct SIR_SBM_Network
  {
    ...
    SIR_SBM_Network(const SBM_Graph_t &G, float p_I0, float p_R, sycl::queue &q,
                    uint32_t seed = 52, uint32_t N_wg = 1024, float p_R0 = .0f);
    ...
    sycl::buffer<uint32_t, 2> community_recoveries_buf;
    ...
    sycl::event recover(uint32_t t, sycl::event &dep_event);
    sycl::event infect(uint32_t t, sycl::event &dep_event);

    sycl::event advance(uint32_t t, sycl::event dep_event = {});

    typedef std::vector<State_t> Community_States_t;
    typedef std::vector<Community_States_t> Community_Trajectory_t;
    std::vector<Edge_t>
    sample_connection_infections(uint32_t community_idx, uint32_t N_infected,
    ...
    std::vector<std::vector<Edge_t>> sample_connection_infections(
        ...
        uint32_t seed = 0);
    sycl::event
    accumulate_community_state(sycl::event dep_event = {});

    // typedef std::tuple<std::vector<sycl::buffer<float>>,
    // std::vector<sycl::buffer<Edge_t>>, std::vector<sycl::buffer<SIR_State>>,
    // std::vector<sycl::buffer<State_t>>> init_t;

    void sim_init(const std::vector<std::vector<float>> &p_Is);

    typedef std::tuple<std::vector<sycl::buffer<State_t>>,
                       std::vector<sycl::buffer<Edge_t>>,
                       std::vector<sycl::event>>
        trajectory_t;

    sycl::event simulate(const SIR_SBM_Param_t &param);
    std::tuple<std::vector<std::vector<State_t>>,
               std::vector<std::vector<Edge_t>>, std::vector<std::vector<Edge_t>>>
    read_trajectory();
    sycl::event create_vcm(
        const std::vector<std::vector<uint32_t>> &node_lists);

  private:
    sycl::event initialize_ecm_buf();
    sycl::event initialize_vcm_buf();
    std::vector<uint32_t> generate_seeds(uint32_t N_rng, uint32_t seed = 42);
