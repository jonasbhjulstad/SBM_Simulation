...
std::vector<Weighted_Edge_t> complete_ccm(uint32_t N_communities, bool directed = false);
...
std::vector<std::vector<uint32_t>> ccm_weights_from_ecms(const std::vector<std::vector<uint32_t>> &ecms, const std::vector<std::vector<Weighted_Edge_t>> &ccms);
