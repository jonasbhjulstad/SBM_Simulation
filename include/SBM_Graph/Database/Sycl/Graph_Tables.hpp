...
{
    // std::shared_ptr<sycl::buffer<T, N>> make_shared_device_buffer(sycl::queue &q, const std::vector<T> &vec, sycl::range<N> range, sycl::event &res_event)
    auto edge_list = SBM_Graph::read_ecm(p_out, graph);
    auto buf = Buffer_Routines::make_shared_device_buffer<uint32_t,  1>(q, edge_list, sycl::range<1>(edge_list.size()), res_event);
    return buf;
}
}
