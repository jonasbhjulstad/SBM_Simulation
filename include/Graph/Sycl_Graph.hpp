...
struct GraphContainerBase {
  ...
  V vertex_prop(uI_t id) {
    ...
    auto it = std::find_if(derived._vertices.begin(), derived._vertices.end(),
  ...
  const Vertex_t* get_neighbor(const Edge_t& e, uI_t id) const
  {
    if (e.from == id)
    {
      return find(e.to);
  ...
  void sort_vertices() {
    const auto &derived = static_cast<Derived const &>(*this);
    std::sort(derived._vertices.begin(), derived._vertices.end(),
...
template <typename V, typename E, std::unsigned_integral uI_t, uI_t NV, uI_t NE,
          std::unsigned_integral uA_t, template <typename, uA_t> typename Array_t>
...
struct GraphContainer
    : public GraphContainerBase<GraphContainer<V, E, uI_t, Array_t>, uI_t, V, E> {
  using Base = GraphContainerBase<GraphContainer<V, E, uI_t, Array_t>, uI_t, V, E>;
  GraphContainer(uI_t NV_max, uI_t NE_max) : NV_max(NV_max), NE_max(NE_max), vertices(NV_max), edges(NE_max){}
  Array_t<Vertex<V, uI_t>> vertices;
  Array_t<Edge<E, uI_t>> edges;
  uI_t &N_vertices = Base::N_vertices;
  uI_t &N_edges = Base::N_edges;
  auto begin() { return std::begin(vertices); }
  auto end() { return std::begin(vertices) + NV_max; }
  uI_t NV_max, NE_max;
};
...
{
  Graph(uI_t NV_max, uI_t NE_max): GraphContainer<V, E, uI_t, Array_t>(NV_max, NE_max) {}
  using Vertex_t = Vertex<V, uI_t>;
  using Edge_t = Edge<E, uI_t>;
  using Vertex_Prop_t = V;
  using Edge_Prop_t = E;
  ...
  const Array_t<Vertex_t*> neighbors(uI_t idx) const {
    Array_t<Vertex_t*> neighbors(NV_max);
    std::transform(edges.begin(), edges.end(), neighbors.begin(), [idx, this](const Edge_t& e)
    {
      if (is_in_edge(e, idx)) {
        const Vertex_t* p_nv = this->get_neighbor(e, idx);
        if (p_nv != nullptr) {
          return p_nv;
        }
      }
      return (Vertex_t*)nullptr;
    });
    return neighbors;
  }
};
