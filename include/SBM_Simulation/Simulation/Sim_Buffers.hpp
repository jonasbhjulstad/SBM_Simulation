...
struct Sim_Buffers
{
    ...
    Sim_Buffers(sycl::queue &q, Sim_Param p, soci::session &sql, const Dataframe::Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list_undirected, const Dataframe::Dataframe_t<uint32_t, 2> &vcms, Dataframe::Dataframe_t<float, 3> p_Is_init = {});
...
private:
    Dataframe::Dataframe_t<float, 3> generate_random_p_Is(sycl::queue &q, Sim_Param p, soci::session &sql, const Dataframe::Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list_undirected, const Dataframe::Dataframe_t<uint32_t, 2> &vcms);

    void construct_buffers(sycl::queue &q, Sim_Param p, soci::session &sql, const Dataframe::Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list, const Dataframe::Dataframe_t<uint32_t, 2> &vcms, Dataframe::Dataframe_t<float, 3> p_Is_init);

    static Sim_Buffers make_impl(sycl::queue &q, Sim_Param p, soci::session &sql, const Dataframe::Dataframe_t<std::pair<uint32_t, uint32_t>, 2> &edge_list, const Dataframe::Dataframe_t<uint32_t, 2> &vcms, Dataframe::Dataframe_t<float, 3> p_Is_init);
};
