...
template <typename T>
Dataframe::Dataframe_t<T, 4> read_3D_buffer(sycl::queue& q, sycl::buffer<T, 3>& buf, uint32_t N_graphs, std::vector<sycl::event> dep_events)
{
...
template <typename T>
void write_dataframe(const std::string& fname, const Dataframe::Dataframe_t<T, 1>& df, bool append = false, const size_t offset = 0)
{
...
template <typename T>
void write_dataframe(const std::string& dir, const std::string& fname, const Dataframe::Dataframe_t<T, 2>& df, bool append = false, const std::array<std::size_t,2> offsets = {0,0})
{
...
template <typename T>
void write_dataframe(const std::string& fname, const Dataframe::Dataframe_t<T, 2>& df, bool append = false, const std::array<std::size_t,2> offsets = {0,0})
{
    std::fstream f;
    if(append)
    {
...
template <typename T>
void write_dataframe(const std::string& basename, const Dataframe::Dataframe_t<T, 3>& df, bool append = false, std::array<std::size_t, 2> offsets = {0,0})
{
    for(int i = 0; i < df.size(); i++)
    {
...
template <typename T>
void write_dataframe(const std::string& base_dir, const std::string& basename, const Dataframe::Dataframe_t<T, 4>& df, bool append = false, std::array<std::size_t, 2> offsets = {0,0})
{
    for(int i = 0; i < df.size(); i++)
    {
